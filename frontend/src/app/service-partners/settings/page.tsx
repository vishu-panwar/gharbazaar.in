  ...
  useEffect(() => {
    async function fetchProfileData() {
      try {
        ...
        // Fetch service provider profile from API
        const profileResponse = await backendApi.serviceProvider.getMyProfile()
        
        if (profileResponse?.success && profileResponse.data) {
          const providerData = profileResponse.data
          
          // Map service provider data to profile state
          const mappedProfile: UserProfile = {
            name: providerData.user?.name || parsedUser?.name || '',
            email: providerData.user?.email || parsedUser?.email || '',
            phone: parsedUser?.phone || '',
            barCouncilId: '', // Optional field, can be added later
            practiceAreas: providerData.specialization ? [providerData.specialization] : [],
            experience: providerData.experience || 0,
            location: providerData.location || '',
            bio: providerData.description || '',
            avatar: parsedUser?.photoURL,
            verificationStatus: providerData.verified ? 'verified' : 'pending'
          }
          
          setProfile(mappedProfile)
        } else {
          // Fallback to user data if no provider profile exists
          if (parsedUser) {
