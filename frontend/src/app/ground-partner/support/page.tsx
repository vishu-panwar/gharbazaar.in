...
export default function GroundPartnerSupportPage() {
  ...
  const [form, setForm] = useState({
    categoryTitle: 'Ground Operations',
    subCategoryTitle: 'General Support',
    problem: '',
  })
  ...
  const createTicket = async () => {
    ...
    try {
      ...
      const response = await backendApi.tickets.create({
        ...
        userRole: 'ground_partner',
      })
      if (!response?.success) {
        throw new Error(response?.message || response?.error || 'Failed to create support ticket')
      }
      toast.success('Support ticket created')
      setForm({ categoryTitle: 'Ground Operations', subCategoryTitle: 'General Support', problem: '' })
      await loadTickets()
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400">
            Ground partner tickets handled through employee and admin support workflow
          </p>
        </div>
        <button
          onClick={loadTickets}
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-4 text-sm text-gray-600 dark:border-gray-800 dark:bg-gray-950 dark:text-gray-400">
        Support threads here are directly visible to employee/admin ticket workflows, ensuring coordinated resolution.
      </div>
    </div>
  )
}
