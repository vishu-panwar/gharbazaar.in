...
export default function SalaryManagementPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [empRes, salRes] = await Promise.all([
        ...
        backendApi.admin.getSalary({ month: monthStr, year })
      ])
  ...
  // Combine employees with their salary record if it exists
  const combinedRecords = employees.map((emp: any) => {
    const record = salaryRecords.find((r: any) => r.userId === emp.uid)
    return {
      id: emp.uid,
      ...
      baseSalary: emp.employeeProfile?.baseSalary || 25000,
      allowances: emp.employeeProfile?.allowances || 0,
      deductions: emp.employeeProfile?.deductions || 0,
      netSalary: record ? record.netSalary : (emp.employeeProfile?.baseSalary || 25000) + (emp.employeeProfile?.allowances || 0) - (emp.employeeProfile?.deductions || 0),
      status: record ? record.status : 'pending',
      paymentDate: record ? new Date(record.paymentDate).toLocaleDateString() : null,
      paymentMethod: record ? record.paymentMethod : null,
      month: selectedMonth,
  ...
  const stats = {
    totalPayroll: combinedRecords.reduce((acc: number, curr: any) => acc + curr.netSalary, 0),
    paidThisMonth: combinedRecords.filter((r: any) => r.status === 'paid').reduce((acc: number, curr: any) => acc + curr.netSalary, 0),
    pendingPayments: combinedRecords.filter((r: any) => r.status === 'pending').length,
    averageSalary: combinedRecords.length > 0 ? combinedRecords.reduce((acc: number, curr: any) => acc + curr.netSalary, 0) / combinedRecords.length : 0,
    payrollGrowth: 0,
  }
  ...
  const handlePaySalary = async () => {
    ...
    try {
      ...
      const response = await backendApi.admin.processSalary({
      ...
      if (response.success) {
        toast.success(`Salary processed for ${selectedEmployee.name}`)
        setShowPaymentModal(false)
        fetchData()
      } else {
        toast.error(response.error || 'Failed to process salary')
      }
    } catch (error) {
  ...
  const handleBulkPayment = async () => {
    const pending = combinedRecords.filter((r: any) => r.status === 'pending')
    if (pending.length === 0) {
      toast.error('No pending payments for this month')
      return
    }

    if (!confirm(`Are you sure you want to process payments for ${pending.length} employees?`)) return
    ...
    try {
      ...
      for (const emp of pending) {
        const res = await backendApi.admin.processSalary({
          userId: emp.id,
          month: monthStr,
          year,
      ...
      toast.success(`Processed ${successCount} payments successfully`)
      fetchData()
    } catch (error) {
      console.error('Error in bulk payment:', error)
      toast.error('Partial failure in bulk processing')
    } finally {
      setPaymentLoading(false)
    }
  }
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3">
          <button
            ...
            className="flex items-center space-x-2 bg-green-600 hover:bg-green-700 disabled:opacity-50 text-white px-6 py-3 rounded-xl font-semibold transition-all shadow-lg"
          >
            {paymentLoading ? <Loader2 className="animate-spin" size={20} /> : <Send size={20} />}
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex items-center space-x-3 pt-6 border-t border-gray-200 dark:border-gray-800">
                <button
                  ...
                  className="flex-1 bg-green-600 hover:bg-green-700 disabled:opacity-50 text-white px-6 py-3 rounded-xl font-semibold transition-all flex items-center justify-center space-x-2"
                >
                  {paymentLoading && <Loader2 className="animate-spin" size={20} />}
