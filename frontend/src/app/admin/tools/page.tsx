...
export default function AdminToolsPage() {
  ...
  const fetchBackups = async () => {
    try {
      const response = await backendApi.admin.database.listBackups()
      if (response.success) {
  ...
  const handleDatabaseBackup = async () => {
    setLoading(true)
    try {
      const response = await backendApi.admin.database.createBackup()
      if (response.success) {
  ...
  const handleDatabaseRestore = (fileName: string) => {
    handleConfirmAction({
      title: 'Restore Database',
      message: `Are you sure you want to restore the database from ${fileName}? This will overwrite current data.`,
      successMessage: 'Database restored successfully',
      onExecute: async () => {
        const response = await backendApi.admin.database.restoreBackup(fileName)
  ...
  const handleExportData = async () => {
    setLoading(true)
    try {
      const response = await backendApi.admin.database.export()
      if (response.success) {
        const dataStr = "data:text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(response.data))
        const downloadAnchorNode = document.createElement('a')
        downloadAnchorNode.setAttribute("href", dataStr)
        downloadAnchorNode.setAttribute("download", `gharbazaar_backup_${new Date().toISOString()}.json`)
    ...
    input.onchange = async (e: any) => {
      ...
      reader.onload = async (event: any) => {
        try {
          ...
          handleConfirmAction({
            title: 'Import Data',
            message: 'Are you sure you want to import this data? Current data will be merged/overwritten.',
            successMessage: 'Data imported successfully',
            onExecute: async () => {
              const response = await backendApi.admin.database.import(backupData)
              if (!response.success) throw new Error(response.error)
            }
          })
        } catch (error) {
          ...
          {[
            ...
            return (
              <button
                ...
                className={`flex items-center space-x-2 px-4 py-3 rounded-xl font-medium transition-all whitespace-nowrap ${activeTab === tab.id
        ...
        <div className="p-6">
          ...
          {activeTab === 'database' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-gray-50 dark:bg-gray-900 rounded-xl p-6 border border-gray-200 dark:border-gray-800">
                  ...
                  <div className="space-y-3">
                    ...
                    <button
                      onClick={() => {
                        // For this button, maybe we just want to open the import/list?
                        // Actually, this button in "Database Operations" seems to be a general restore.
                        // I'll make it prompt for a file or just refer to the list below.
                        toast('Please use the restore button in the Server Backups list below.', { icon: 'ℹ️' })
                      }}
              ...
              <div className="bg-white dark:bg-gray-950 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden mt-6">
                <div className="p-4 border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50">
                  <h3 className="font-bold text-gray-900 dark:text-white">Server Backups</h3>
                </div>
                <div className="divide-y divide-gray-200 dark:divide-gray-800">
                  {backups.length === 0 ? (
                    ...
                    backups.map((backup) => (
                      <div key={backup.fileName} className="p-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors">
                        ...
                        <button
                          onClick={() => handleDatabaseRestore(backup.fileName)}
                          className="px-3 py-1.5 text-sm font-medium text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-all"
                        >
                          Restore
                        </button>
                      </div>
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-yellow-700 dark:text-yellow-400">
                      Regular backups are recommended. Last backup was {backups.length > 0 ? new Date(backups[0].createdAt).toLocaleDateString() : 'never'}.
                      Total server backups: {backups.length}
                    </p>
                  </div>
                </div>
              </div>
