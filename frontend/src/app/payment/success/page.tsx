...
export default function PaymentSuccessPage() {
  ...
  const downloadInvoice = () => {
    ...
    showAlert({
      ...
      message: 'Your invoice is being generated and will be sent to your email shortly.',
      type: 'info'
    })
  }
              ...
              <div className="p-6">
                <div className="space-y-6">
                  {nextSteps.map((step, index) => (
                    <div
                      key={step.step}
                      className={`flex items-start space-x-4 transition-all duration-500 ${showTimeline ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-4'
                        }`}
                      style={{ transitionDelay: `${index * 200}ms` }}
                    >
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${step.status === 'pending'
