...
export default function NewListingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Import backendApi dynamically to avoid SSR issues
      ...
      const uploadedImageConfigs = await Promise.all(
        images.map(async (file) => {
          try {
            const response = await backendApi.properties.uploadImage(file)
      ...
      const validImageUrls = uploadedImageConfigs.filter(url => url !== null) as string[]
