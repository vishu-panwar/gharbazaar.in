...
export default function DashboardPricingPage() {
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      try {
        ...
        const buyerPlans = rawPlans.filter(
          (plan: BackendPlan) => plan.type === 'buyer' || plan.type === 'combined'
        );
  ...
  const handlePaymentSuccess = async (paymentData: any) => {
    ...
    try {
      ...
      showAlert({
        ...
        message: `${currentPlan.displayName || currentPlan.name} is now active.`,
        type: 'success',
      });
      ...
      showAlert({
        ...
        message: error?.message || 'Payment succeeded but plan activation failed.',
        type: 'error',
      });
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white dark:bg-gray-900 rounded-2xl p-6 border border-gray-200 dark:border-gray-800">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Buyer Plans</h1>
        <p className="text-gray-600 dark:text-gray-400 mt-2">
          Select an active buyer plan to unlock property details and direct seller inquiries.
        </p>
      </div>
      ...
      {loadingPlans ? (
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl p-8 border border-gray-200 dark:border-gray-800 text-center text-gray-500">
          No buyer plans are available right now.
        </div>
      ) : (
          ...
          {sortedPlans.map((plan) => (
            ...
            >
              ...
              <button
                ...
                className="mt-6 w-full py-3 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold"
              >
